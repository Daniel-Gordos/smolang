...
public:
    ...
    void
    parse(int argc, const char *const argv[], int mode = PREFER_FLAG_FOR_UNREG_OPTION);
    ...
    template<typename T>
    string_stream
    operator()(std::initializer_list<char const *const> init_list, T &&def_val) const;
    ...
    int argc, const char *const argv[], int mode /*= PREFER_FLAG_FOR_UNREG_OPTION*/)
{
    ...
    for (auto i = 0u; i < args_.size(); ++i)
    {
        ...
        assert(
            !(mode & argh::parser::PREFER_FLAG_FOR_UNREG_OPTION) ||
...
template<typename T>
string_stream
parser::operator()(std::initializer_list<char const *const> init_list, T &&def_val) const
{
    for (auto &name: init_list)
    {
        auto optIt = params_.find(trim_leading_dashes(name));
...
inline multimap_iteration_wrapper parser::params(std::string const &name) const
{
    ...
    return multimap_iteration_wrapper(
        params_.lower_bound(trimmed_name), params_.upper_bound(trimmed_name));
}
